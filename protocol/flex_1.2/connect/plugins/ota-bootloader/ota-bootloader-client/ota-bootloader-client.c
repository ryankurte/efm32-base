...
#include "ota-bootloader-client.h"
#include "ota-bootloader-client-internal.h"
                                                                              ...
                                                                              uint8_t applicationErrorStatus)
{
  if (clientInImageDownload()
    ...
    historyTableAddEntry(EMBER_OTA_BOOTLOADER_PROTOCOL_FRAME_CONTROL_RESP_STATUS_ABORTED,
                         applicationErrorStatus);
    imageDownloadFinished(EMBER_OTA_BOOTLOADER_STATUS_ABORTED);
...
{
  if (message->endpoint != EMBER_AF_PLUGIN_OTA_BOOTLOADER_CLIENT_ENDPOINT) {
    return;
  }

#if defined(EMBER_AF_PLUGIN_OTA_BOOTLOADER_CLIENT_SECURITY_ENABLED)
  // If security is enabled, discard all unsecured messages.
  if (!(message->options & EMBER_OPTIONS_SECURITY_ENABLED)) {
    return;
  }
#endif // EMBER_AF_PLUGIN_OTA_BOOTLOADER_CLIENT_SECURITY_ENABLED
...
void emAfPluginOtaBootloaderClientEventHandler(void)
{
  ...
  if (internalState == STATE_OTA_CLIENT_IMAGE_DOWNLOAD_WAITING) {
    historyTableAddEntry(EMBER_OTA_BOOTLOADER_PROTOCOL_FRAME_CONTROL_RESP_STATUS_FAILED,
                         EMBER_AF_PLUGIN_OTA_BOOTLOADER_INVALID_APPLICATION_TARGET_STATUS);
    imageDownloadFinished(EMBER_OTA_BOOTLOADER_STATUS_TIMEOUT);
  }
}
...
static void imageDownloadFinished(EmberAfOtaBootloaderStatus status)
{
  ...
  emberAfPluginOtaBootloaderClientImageDownloadCompleteCallback(status,
                                                                imageInfo.tag,
                                                                ((status == EMBER_OTA_BOOTLOADER_STATUS_SUCCESS)
                                                                 ? imageInfo.size
                                                                 : 0));
}
...
static void handleIncomingImageSegment(EmberIncomingMessage *message)
{
  ...
  uint8_t imageTag =
    message->payload[EMBER_OTA_BOOTLOADER_PROTOCOL_IMAGE_SEGMENT_TAG_OFFSET];
  uint16_t segmentIndex =
    emberFetchLowHighInt16u(message->payload
                            + EMBER_OTA_BOOTLOADER_PROTOCOL_IMAGE_SEGMENT_INDEX_OFFSET);
  ...
  if (blockIndex == 0 && clientIsIdle()) {
    ...
    if (segmentBitmaskBuffer == EMBER_NULL_BUFFER
      ...
      segmentBitmaskBuffer = EMBER_NULL_BUFFER;
      historyTableAddEntry(EMBER_OTA_BOOTLOADER_PROTOCOL_FRAME_CONTROL_RESP_STATUS_REFUSED,
                           EMBER_AF_PLUGIN_OTA_BOOTLOADER_INVALID_APPLICATION_TARGET_STATUS);
      return;
    }
    ...
    if (blockIndex == imageInfo.blockIndex) {
      ...
      if (!getSegmentBit(segmentIndex)) {
        emberAfPluginOtaBootloaderClientIncomingImageSegmentCallback(serverId,
                                                                     ...
                                                                     imageTag,
                                                                     message->payload + EMBER_OTA_BOOTLOADER_PROTOCOL_IMAGE_SEGMENT_PAYLOAD_OFFSET);
      ...
      emberEventControlSetDelayQS(emAfPluginOtaBootloaderClientEventControl,
                                  EMBER_AF_PLUGIN_OTA_BOOTLOADER_CLIENT_IMAGE_DOWNLOAD_TIMEOUT_S * 4);
      ...
      historyTableAddEntry(EMBER_OTA_BOOTLOADER_PROTOCOL_FRAME_CONTROL_RESP_STATUS_FAILED,
                           EMBER_AF_PLUGIN_OTA_BOOTLOADER_INVALID_APPLICATION_TARGET_STATUS);
      imageDownloadFinished(EMBER_OTA_BOOTLOADER_STATUS_FAILED);
    }
  }
}
...
{
  uint8_t message[EMBER_OTA_BOOTLOADER_PROTOCOL_MISSING_SEGS_RESP_HEADER_LENGTH];

  // Frame control
  message[EMBER_OTA_BOOTLOADER_PROTOCOL_FRAME_CONTROL_OFFSET] =
    ((EMBER_OTA_BOOTLOADER_PROTOCOL_COMMAND_ID_MISSING_SEGMENTS_RESPONSE
      << EMBER_OTA_BOOTLOADER_PROTOCOL_FRAME_CONTROL_COMMAND_ID_OFFSET)
     | (otaProtocolStatus
        << EMBER_OTA_BOOTLOADER_PROTOCOL_FRAME_CONTROL_RESP_STATUS_OFFSET));
  // Application status
  message[EMBER_OTA_BOOTLOADER_PROTOCOL_MISSING_SEGS_RESP_APP_STATUS_OFFSET] =
    applicationStatus;
  // Image tag
  message[EMBER_OTA_BOOTLOADER_PROTOCOL_MISSING_SEGS_RESP_TAG_OFFSET] =
    imageTag;

  if (allZerosBitmask) {
    MEMSET(message + EMBER_OTA_BOOTLOADER_PROTOCOL_MISSING_SEGS_RESP_BITMASK_OFFSET,
           0x00,
    ...
    for (i = 0; i < MISSING_SEGMENTS_BITMASK_LENGTH; i++) {
      message[EMBER_OTA_BOOTLOADER_PROTOCOL_MISSING_SEGS_RESP_BITMASK_OFFSET + i] =
  ...
  emberMessageSend(serverId,
                   EMBER_AF_PLUGIN_OTA_BOOTLOADER_CLIENT_ENDPOINT,
                   0, // messageTag
                   EMBER_OTA_BOOTLOADER_PROTOCOL_MISSING_SEGS_RESP_HEADER_LENGTH,
                   message,
                   UNICAST_TX_OPTIONS);
}
...
{
  uint8_t imageTag =
    message->payload[EMBER_OTA_BOOTLOADER_PROTOCOL_MISSING_SEGS_REQ_TAG_OFFSET];
  uint32_t imageSize = emberFetchLowHighInt32u(message->payload
                                               + EMBER_OTA_BOOTLOADER_PROTOCOL_MISSING_SEGS_REQ_IMAGE_SIZE_OFFSET);
  uint16_t firstSegmentIndex =
    emberFetchLowHighInt16u(message->payload
                            + EMBER_OTA_BOOTLOADER_PROTOCOL_MISSING_SEGS_REQ_FIRST_INDEX_OFFSET);
  uint16_t lastSegmentIndex =
    emberFetchLowHighInt16u(message->payload
                            + EMBER_OTA_BOOTLOADER_PROTOCOL_MISSING_SEGS_REQ_LAST_INDEX_OFFSET);
  ...
  if (clientInImageDownload()
    ...
    if (blockIndex <= imageInfo.blockIndex) {
      ...
      if (blockCompleted) {
        ...
        if (isLastBlock) {
          historyTableAddEntry(EMBER_OTA_BOOTLOADER_PROTOCOL_FRAME_CONTROL_RESP_STATUS_COMPLETED,
                               EMBER_AF_PLUGIN_OTA_BOOTLOADER_INVALID_APPLICATION_TARGET_STATUS);
          imageDownloadFinished(EMBER_OTA_BOOTLOADER_STATUS_SUCCESS);
        } else {
  ...
  // otherwise send a default response.
  {
    ...
    if (historyTableEntryIndex == 0xFF) {
      ...
      // values.
      otaProtocolStatus = EMBER_OTA_BOOTLOADER_PROTOCOL_FRAME_CONTROL_RESP_STATUS_FAILED;
      applicationErrorStatus = EMBER_AF_PLUGIN_OTA_BOOTLOADER_INVALID_APPLICATION_TARGET_STATUS;
    } else {
...
static void sendTargetStatusResponse(EmberNodeId serverId,
                                     uint8_t applicationStatus)
{
  uint8_t message[EMBER_OTA_BOOTLOADER_PROTOCOL_TARGET_STATUS_RESP_HEADER_LENGTH];

  // Frame control
  message[EMBER_OTA_BOOTLOADER_PROTOCOL_FRAME_CONTROL_OFFSET] =
    ((EMBER_OTA_BOOTLOADER_PROTOCOL_COMMAND_ID_TARGET_STATUS_RESPONSE
      << EMBER_OTA_BOOTLOADER_PROTOCOL_FRAME_CONTROL_COMMAND_ID_OFFSET)
     | (EMBER_OTA_BOOTLOADER_PROTOCOL_FRAME_CONTROL_RESP_STATUS_COMPLETED
        << EMBER_OTA_BOOTLOADER_PROTOCOL_FRAME_CONTROL_RESP_STATUS_OFFSET));
  // Application status
  message[EMBER_OTA_BOOTLOADER_PROTOCOL_TARGET_STATUS_RESP_APP_STATUS_OFFSET] =
    applicationStatus;
  ...
  emberMessageSend(serverId,
                   EMBER_AF_PLUGIN_OTA_BOOTLOADER_CLIENT_ENDPOINT,
                   0, // messageTag
                   EMBER_OTA_BOOTLOADER_PROTOCOL_TARGET_STATUS_RESP_HEADER_LENGTH,
                   message,
                   UNICAST_TX_OPTIONS);
}
...
static void handleIncomingTargetStatusRequest(EmberIncomingMessage *message)
{
  uint8_t serverApplicationStatus =
    message->payload[EMBER_OTA_BOOTLOADER_PROTOCOL_TARGET_STATUS_REQ_APP_STATUS_OFFSET];
...
static void sendBootloadResponse(EmberNodeId serverId,
                                 ...
                                 bool accepted)
{
  uint8_t message[EMBER_OTA_BOOTLOADER_PROTOCOL_BOOTLOAD_RESP_HEADER_LENGTH];

  // Frame control
  message[EMBER_OTA_BOOTLOADER_PROTOCOL_FRAME_CONTROL_OFFSET] =
    ((EMBER_OTA_BOOTLOADER_PROTOCOL_COMMAND_ID_BOOTLOAD_RESPONSE
      << EMBER_OTA_BOOTLOADER_PROTOCOL_FRAME_CONTROL_COMMAND_ID_OFFSET)
     | (((accepted)
         ? EMBER_OTA_BOOTLOADER_PROTOCOL_FRAME_CONTROL_RESP_STATUS_COMPLETED
        ...
        << EMBER_OTA_BOOTLOADER_PROTOCOL_FRAME_CONTROL_RESP_STATUS_OFFSET));
  // Image tag
  message[EMBER_OTA_BOOTLOADER_PROTOCOL_BOOTLOAD_RESP_TAG_OFFSET] = imageTag;
  // Application status
  message[EMBER_OTA_BOOTLOADER_PROTOCOL_BOOTLOAD_RESP_APP_STATUS_OFFSET] =
    applicationStatus;
  ...
  emberMessageSend(serverId,
                   EMBER_AF_PLUGIN_OTA_BOOTLOADER_CLIENT_ENDPOINT,
                   0,  // messageTag
                   EMBER_OTA_BOOTLOADER_PROTOCOL_BOOTLOAD_RESP_HEADER_LENGTH,
                   message,
                   UNICAST_TX_OPTIONS);
}
...
static void handleIncomingBootloadRequest(EmberIncomingMessage *message)
{
  uint8_t applicationStatus;
  uint8_t imageTag =
    message->payload[EMBER_OTA_BOOTLOADER_PROTOCOL_BOOTLOAD_REQ_TAG_OFFSET];
  uint32_t delayMs = emberFetchLowHighInt32u(message->payload
                                             + EMBER_OTA_BOOTLOADER_PROTOCOL_BOOTLOAD_REQ_DELAY_OFFSET);
  bool accepted =
