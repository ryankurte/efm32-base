...
#include "ota-bootloader-server.h"
#include "ota-bootloader-server-internal.h"
                                                                                     ...
                                                                                     uint16_t targetListLength)
{
  ...
  if (!serverIsIdle()) {
    return EMBER_OTA_BOOTLOADER_STATUS_BUSY;
  }
  ...
  if (imageSize == 0
      ...
      || targetList == NULL
      || targetListLength == 0) {
    return EMBER_OTA_BOOTLOADER_STATUS_INVALID_CALL;
  }
  ...
  if (targetListBuffer == EMBER_NULL_BUFFER
    ...
    return EMBER_OTA_BOOTLOADER_STATUS_NO_BUFFERS;
  }
  ...
  return EMBER_OTA_BOOTLOADER_STATUS_SUCCESS;
}
                                                                              ...
                                                                              uint8_t *applicationTargetStatus)
{
  ...
  if (serverIsIdle() && !serverIsCompletingProcess()) {
    return EMBER_OTA_BOOTLOADER_TARGET_STATUS_INVALID;
  }
  ...
  *applicationTargetStatus = *emGetTargetApplicationStatus(targetIndex);
  ...
  if (serverInImageDistributionProcess()
    ...
    if (emTargetIsUnreachable(targetIndex)) {
      return EMBER_OTA_BOOTLOADER_TARGET_STATUS_UNREACHABLE;
    } else if (otaStatus
               == EMBER_OTA_BOOTLOADER_PROTOCOL_FRAME_CONTROL_RESP_STATUS_COMPLETED) {
      return EMBER_OTA_BOOTLOADER_TARGET_STATUS_DISTRIBUTION_COMPLETED;
    } else if (otaStatus
               == EMBER_OTA_BOOTLOADER_PROTOCOL_FRAME_CONTROL_RESP_STATUS_ONGOING) {
      return EMBER_OTA_BOOTLOADER_TARGET_STATUS_DISTRIBUTION_ONGOING;
    } else if (otaStatus
               == EMBER_OTA_BOOTLOADER_PROTOCOL_FRAME_CONTROL_RESP_STATUS_FAILED) {
      return EMBER_OTA_BOOTLOADER_TARGET_STATUS_DISTRIBUTION_FAILED;
    } else if (otaStatus
               == EMBER_OTA_BOOTLOADER_PROTOCOL_FRAME_CONTROL_RESP_STATUS_REFUSED) {
      return EMBER_OTA_BOOTLOADER_TARGET_STATUS_DISTRIBUTION_REFUSED;
    } else if (otaStatus
               == EMBER_OTA_BOOTLOADER_PROTOCOL_FRAME_CONTROL_RESP_STATUS_ABORTED) {
      return EMBER_OTA_BOOTLOADER_TARGET_STATUS_DISTRIBUTION_ABORTED;
    }
  }
  ...
  if (serverInTargetStatusRequestProcess()
    ...
    if (emTargetIsUnreachable(targetIndex)) {
      return EMBER_OTA_BOOTLOADER_TARGET_STATUS_UNREACHABLE;
               ...
               == EMBER_OTA_BOOTLOADER_PROTOCOL_FRAME_CONTROL_RESP_STATUS_COMPLETED) {
      return EMBER_OTA_BOOTLOADER_TARGET_STATUS_STATUS_REQUEST_COMPLETED;
    } else if (otaStatus
               == EMBER_OTA_BOOTLOADER_PROTOCOL_FRAME_CONTROL_RESP_STATUS_ONGOING) {
      return EMBER_OTA_BOOTLOADER_TARGET_STATUS_STATUS_REQUEST_ONGOING;
    }
  }
  ...
  if (serverInBootloadRequestProcess()
    ...
    if (emTargetIsUnreachable(targetIndex)) {
      return EMBER_OTA_BOOTLOADER_TARGET_STATUS_UNREACHABLE;
               ...
               == EMBER_OTA_BOOTLOADER_PROTOCOL_FRAME_CONTROL_RESP_STATUS_COMPLETED) {
      return EMBER_OTA_BOOTLOADER_TARGET_STATUS_BOOTLOAD_REQUEST_ACCEPTED;
    } else if (otaStatus
               == EMBER_OTA_BOOTLOADER_PROTOCOL_FRAME_CONTROL_RESP_STATUS_ONGOING) {
      return EMBER_OTA_BOOTLOADER_TARGET_STATUS_BOOTLOAD_REQUEST_ONGOING;
    } else if (otaStatus
               == EMBER_OTA_BOOTLOADER_PROTOCOL_FRAME_CONTROL_RESP_STATUS_REFUSED) {
      return EMBER_OTA_BOOTLOADER_TARGET_STATUS_BOOTLOAD_REQUEST_REFUSED;
    }
  }
...
{
  if (serverInImageDistributionProcess()) {
    imageDistributionProcessFinished(EMBER_OTA_BOOTLOADER_STATUS_ABORTED);

    return EMBER_OTA_BOOTLOADER_STATUS_SUCCESS;
  }

  if (serverInTargetStatusRequestProcess()) {
    targetsStatusRequestProcessFinished(EMBER_OTA_BOOTLOADER_STATUS_ABORTED);

    return EMBER_OTA_BOOTLOADER_STATUS_SUCCESS;
  }

  if (serverInBootloadRequestProcess()) {
    bootloadRequestProcessFinished(EMBER_OTA_BOOTLOADER_STATUS_ABORTED);
...
{
  if (message->endpoint != EMBER_AF_PLUGIN_OTA_BOOTLOADER_SERVER_ENDPOINT) {
    return;
  }

#if defined(EMBER_AF_PLUGIN_OTA_BOOTLOADER_SERVER_SECURITY_ENABLED)
  // If security is enabled, discard all unsecured messages.
  if (!(message->options & EMBER_OPTIONS_SECURITY_ENABLED)) {
    return;
  }
#endif // EMBER_AF_PLUGIN_OTA_BOOTLOADER_SERVER_SECURITY_ENABLED

  switch (emOtaBootloaderProtocolCommandId(message->payload)) {
    case EMBER_OTA_BOOTLOADER_PROTOCOL_COMMAND_ID_MISSING_SEGMENTS_RESPONSE:
      if (internalState == STATE_OTA_SERVER_MISSING_SEGMENTS_WAITING_RESPONSE
          && message->length == EMBER_OTA_BOOTLOADER_PROTOCOL_MISSING_SEGS_RESP_HEADER_LENGTH) {
        ...
        *emGetTargetApplicationStatus(targetIndex) =
          message->payload[EMBER_OTA_BOOTLOADER_PROTOCOL_MISSING_SEGS_RESP_APP_STATUS_OFFSET];
        ...
        if (emOtaBootloaderProtocolResponseStatus(message->payload)
          ...
          processMissingSegmentsBitmask(message->payload
                                        + EMBER_OTA_BOOTLOADER_PROTOCOL_MISSING_SEGS_RESP_BITMASK_OFFSET);
        }

        scheduleImageDistributionProcessNextTask(true);
      }
      break;
    case EMBER_OTA_BOOTLOADER_PROTOCOL_COMMAND_ID_TARGET_STATUS_RESPONSE:
      if (internalState == STATE_OTA_SERVER_TARGET_STATUS_REQUEST_WAITING_RESPONSE
          && message->length == EMBER_OTA_BOOTLOADER_PROTOCOL_TARGET_STATUS_RESP_HEADER_LENGTH) {
        uint16_t targetIndex = emTargetLookup(message->source);
        ...
        // Save the application status.
        *emGetTargetApplicationStatus(targetIndex) =
          message->payload[EMBER_OTA_BOOTLOADER_PROTOCOL_TARGET_STATUS_RESP_APP_STATUS_OFFSET];

        scheduleTargetStatusRequestProcessNextTask(true);
      }
      break;
    case EMBER_OTA_BOOTLOADER_PROTOCOL_COMMAND_ID_BOOTLOAD_RESPONSE:
      if (internalState == STATE_OTA_SERVER_BOOTLOAD_REQUEST_WAITING_RESPONSE
          && message->length == EMBER_OTA_BOOTLOADER_PROTOCOL_BOOTLOAD_RESP_HEADER_LENGTH) {
        uint16_t targetIndex = emTargetLookup(message->source);
        ...
        // Save the application status.
        *emGetTargetApplicationStatus(targetIndex) =
          message->payload[EMBER_OTA_BOOTLOADER_PROTOCOL_BOOTLOAD_RESP_APP_STATUS_OFFSET];
                                                      ...
                                                      EmberOutgoingMessage *message)
{
  ...
  switch (emOtaBootloaderProtocolCommandId(message->payload)) {
    case EMBER_OTA_BOOTLOADER_PROTOCOL_COMMAND_ID_IMAGE_SEGMENT:
      ...
      break;
    case EMBER_OTA_BOOTLOADER_PROTOCOL_COMMAND_ID_MISSING_SEGMENTS_REQUEST:
      ...
      if (status == EMBER_SUCCESS) {
        ...
        emberEventControlSetDelayMS(emAfPluginOtaBootloaderServerEventControl,
                                    EMBER_AF_PLUGIN_OTA_BOOTLOADER_SERVER_RESPONSE_TIMEOUT_MS);
        stackErrorsCount = 0;
      } else {
      ...
      break;
    case EMBER_OTA_BOOTLOADER_PROTOCOL_COMMAND_ID_TARGET_STATUS_REQUEST:
      ...
      break;
    case EMBER_OTA_BOOTLOADER_PROTOCOL_COMMAND_ID_BOOTLOAD_REQUEST:
...
// TODO: these functions can share most of the code.
...
uint8_t *emGetTargetMiscInfo(uint16_t targetIndex)
{
  ...
  return &(targetListBufferPtr[targetIndex * TARGET_LIST_ENTRY_LENGTH
...
static void initTargetList(EmberNodeId *targetList, uint16_t targetListLength)
{
  ...
  for (i = 0; i < targetListLength; i++) {
    ...
    targetListBufferPtr[i * TARGET_LIST_ENTRY_LENGTH
                        + TARGET_LIST_ENTRY_APP_STATUS_OFFSET] =
      EMBER_AF_PLUGIN_OTA_BOOTLOADER_INVALID_APPLICATION_TARGET_STATUS;
    targetListBufferPtr[i * TARGET_LIST_ENTRY_LENGTH
                        + TARGET_LIST_ENTRY_MISC_INFO_OFFSET] =
...
static bool segmentIndexToBit(uint16_t segmentIndex)
{
  ...
  return ((segmentBitmaskBufferPtr[byteIndex] & BIT(bitIndex)) > 0);
}
...
static void broadcastNextSegment(void)
{
  ...
  if (!emberAfPluginOtaBootloaderServerGetImageSegmentCallback(startIndex,
                                                               ...
                                                               message + EMBER_OTA_BOOTLOADER_PROTOCOL_IMAGE_SEGMENT_HEADER_LENGTH)) {
    imageDistributionProcessFinished(EMBER_OTA_BOOTLOADER_STATUS_DATA_UNDERFLOW);
  } else {
    EmberStatus status;

    // Frame control
    message[EMBER_OTA_BOOTLOADER_PROTOCOL_FRAME_CONTROL_OFFSET] =
      EMBER_OTA_BOOTLOADER_PROTOCOL_COMMAND_ID_IMAGE_SEGMENT;
    // Image tag
    message[EMBER_OTA_BOOTLOADER_PROTOCOL_IMAGE_SEGMENT_TAG_OFFSET] =
      currentImageTagOrServerStatus;
    // Segment index
    emberStoreLowHighInt16u(message + EMBER_OTA_BOOTLOADER_PROTOCOL_IMAGE_SEGMENT_INDEX_OFFSET,
                            nextSegmentOrTargetIndex);

    status = emberMessageSend(EMBER_BROADCAST_ADDRESS,
                              EMBER_AF_PLUGIN_OTA_BOOTLOADER_SERVER_ENDPOINT,
                              0, // messageTag
                              (EMBER_OTA_BOOTLOADER_PROTOCOL_IMAGE_SEGMENT_HEADER_LENGTH
...
static void queryNextTargetForMissingSegments(void)
{
  uint8_t message[EMBER_OTA_BOOTLOADER_PROTOCOL_MISSING_SEGS_REQ_HEADER_LENGTH];
  EmberStatus status;
  ...
  // Frame control
  message[EMBER_OTA_BOOTLOADER_PROTOCOL_FRAME_CONTROL_OFFSET] =
    EMBER_OTA_BOOTLOADER_PROTOCOL_COMMAND_ID_MISSING_SEGMENTS_REQUEST;
  // Image tag
  message[EMBER_OTA_BOOTLOADER_PROTOCOL_MISSING_SEGS_REQ_TAG_OFFSET] =
    currentImageTagOrServerStatus;
  // Image size
  emberStoreLowHighInt32u(message + EMBER_OTA_BOOTLOADER_PROTOCOL_MISSING_SEGS_REQ_IMAGE_SIZE_OFFSET,
                          currentImageSizeOrBootloadTimeMs);
  // First segment index
  emberStoreLowHighInt16u(message + EMBER_OTA_BOOTLOADER_PROTOCOL_MISSING_SEGS_REQ_FIRST_INDEX_OFFSET,
                          firstSegmentIndex);
  // Last segment index
  emberStoreLowHighInt16u(message + EMBER_OTA_BOOTLOADER_PROTOCOL_MISSING_SEGS_REQ_LAST_INDEX_OFFSET,
                          lastSegmentIndex);

  status = emberMessageSend(destination,
                            EMBER_AF_PLUGIN_OTA_BOOTLOADER_SERVER_ENDPOINT,
                            0, // messageTag
                            EMBER_OTA_BOOTLOADER_PROTOCOL_MISSING_SEGS_REQ_HEADER_LENGTH,
                            message,
                            UNICAST_TX_OPTIONS);
...
static void scheduleImageDistributionProcessNextTask(bool newSegmentOrNewTarget)
{
  switch (internalState) {
    ...
    case STATE_OTA_SERVER_SEGMENT_BROADCAST_PENDING:
      if (stackErrorsCount >= EMBER_AF_PLUGIN_OTA_BOOTLOADER_SERVER_MAX_STACK_ERRORS) {
        imageDistributionProcessFinished(EMBER_OTA_BOOTLOADER_STATUS_STACK_ERROR);
        return;
      }
      ...
      emberEventControlSetDelayMS(emAfPluginOtaBootloaderServerEventControl,
                                  EMBER_AF_PLUGIN_OTA_BOOTLOADER_SERVER_TX_INTERVAL_MS);
      break;
    case STATE_OTA_SERVER_MISSING_SEGMENTS_UNICAST_INTERVAL:
    ...
    case STATE_OTA_SERVER_MISSING_SEGMENTS_WAITING_RESPONSE:
      ...
      if (currentTargetErrorsCount
          >= EMBER_AF_PLUGIN_OTA_BOOTLOADER_SERVER_MAX_UNICAST_ERRORS) {
      ...
      if (updateNextTargetIndex(newSegmentOrNewTarget)) {
        ...
        if (!updateNextTargetIndex(false)) {
          imageDistributionProcessFinished(EMBER_OTA_BOOTLOADER_STATUS_FAILED);
          return;
        }
        ...
        if (currentBlockCompleted()) {
          ...
          if (currentBlockBroadcastRoundsCount
            ...
            imageDistributionProcessFinished(EMBER_OTA_BOOTLOADER_STATUS_TIMEOUT);
            return;
          }
        }
        ...
        if (currentBlockIndex < getTotalBlocksCount()) {
          ...
          imageDistributionProcessFinished(EMBER_OTA_BOOTLOADER_STATUS_SUCCESS);
        }
      }
...
static void queryNextTargetForTargetStatus(void)
{
  uint8_t message[EMBER_OTA_BOOTLOADER_PROTOCOL_TARGET_STATUS_REQ_HEADER_LENGTH];
  EmberStatus status;
  EmberNodeId destination;
  uint8_t *targetListBufferPtr;
  ...
  // Frame control
  message[EMBER_OTA_BOOTLOADER_PROTOCOL_FRAME_CONTROL_OFFSET] =
    EMBER_OTA_BOOTLOADER_PROTOCOL_COMMAND_ID_TARGET_STATUS_REQUEST;
  // Application status
  message[EMBER_OTA_BOOTLOADER_PROTOCOL_TARGET_STATUS_REQ_APP_STATUS_OFFSET] =
    currentImageTagOrServerStatus;

  status = emberMessageSend(destination,
                            EMBER_AF_PLUGIN_OTA_BOOTLOADER_SERVER_ENDPOINT,
                            0, // messageTag
                            EMBER_OTA_BOOTLOADER_PROTOCOL_TARGET_STATUS_REQ_HEADER_LENGTH,
                            message,
                            UNICAST_TX_OPTIONS);
...
static void scheduleTargetStatusRequestProcessNextTask(bool newTarget)
{
  ...
  if (stackErrorsCount >= EMBER_AF_PLUGIN_OTA_BOOTLOADER_SERVER_MAX_STACK_ERRORS) {
    targetsStatusRequestProcessFinished(EMBER_OTA_BOOTLOADER_STATUS_STACK_ERROR);
    return;
  }
  ...
  if (updateNextTargetIndex(newTarget)) {
    ...
    targetsStatusRequestProcessFinished(EMBER_OTA_BOOTLOADER_STATUS_SUCCESS);
  }
}
...
static void requestNextTargetForBootload(void)
{
  uint8_t message[EMBER_OTA_BOOTLOADER_PROTOCOL_BOOTLOAD_REQ_HEADER_LENGTH];
  ...
  // Frame control
  message[EMBER_OTA_BOOTLOADER_PROTOCOL_FRAME_CONTROL_OFFSET] =
    EMBER_OTA_BOOTLOADER_PROTOCOL_COMMAND_ID_BOOTLOAD_REQUEST;
  // Image tag
  message[EMBER_OTA_BOOTLOADER_PROTOCOL_BOOTLOAD_REQ_TAG_OFFSET] =
    currentImageTagOrServerStatus;
  // Delay
  emberStoreLowHighInt32u(message + EMBER_OTA_BOOTLOADER_PROTOCOL_BOOTLOAD_REQ_DELAY_OFFSET,
                          delayMs);

  status = emberMessageSend(destination,
                            EMBER_AF_PLUGIN_OTA_BOOTLOADER_SERVER_ENDPOINT,
                            0, // messageTag
                            EMBER_OTA_BOOTLOADER_PROTOCOL_BOOTLOAD_REQ_HEADER_LENGTH,
                            message,
                            UNICAST_TX_OPTIONS);
...
static void scheduleBootloadRequestProcessNextTask(bool newTarget)
{
  ...
  if (stackErrorsCount >= EMBER_AF_PLUGIN_OTA_BOOTLOADER_SERVER_MAX_STACK_ERRORS) {
    bootloadRequestProcessFinished(EMBER_OTA_BOOTLOADER_STATUS_STACK_ERROR);
    return;
  }
  ...
  if (updateNextTargetIndex(newTarget)) {
    ...
    bootloadRequestProcessFinished(EMBER_OTA_BOOTLOADER_STATUS_SUCCESS);
  }
}
