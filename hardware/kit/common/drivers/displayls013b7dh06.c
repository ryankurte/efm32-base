/***************************************************************************//**
 * @file
 * @brief Display driver for the Sharp Memory LCD LS013B7DH06
 *******************************************************************************
 * # License
...
/* LS013B7DH06 timing */
#define LS013B7DH06_SCS_SETUP_US       3
#define LS013B7DH06_SCS_HOLD_US        1
#define LS013B7DH06_TCOM_LATCH_US     30
#define LS013B7DH06_TCOM_POLARITY_US  30

/* Define time for power supply to ramp after enabled */
#define SHARP_MEMLCD_POWER_RAMP_US    100

#define LS013B7DH06_ENABLE_WAIT_US (SHARP_MEMLCD_POWER_RAMP_US  \
                                    + LS013B7DH06_TCOM_LATCH_US \
                                    + LS013B7DH06_TCOM_POLARITY_US)

/* LS013B7DH06 SPI commands */
#define LS013B7DH06_CMD_UPDATE     (0x01)
#define LS013B7DH06_CMD_ALL_CLEAR  (0x04)

/* Frequency of LCD polarity inversion. */
#ifndef LS013B7DH06_POLARITY_INVERSION_FREQUENCY
#define LS013B7DH06_POLARITY_INVERSION_FREQUENCY (1)
#endif

#ifdef USE_CONTROL_BYTES
#define LS013B7DH06_CONTROL_BYTES     (2)
#else
#define LS013B7DH06_CONTROL_BYTES     (0)
#endif
...
/**************************************************************************//**
 * @brief  Initialize the LS013B7DH06 display driver
 *
...
EMSTATUS DISPLAY_Ls013b7dh06Init(void)
{
  ...
  /* Setup and register the LS013B7DH06 as a DISPLAY device now. */
  ...
  display.geometry.width        = LS013B7DH06_WIDTH;
  display.geometry.height       = LS013B7DH06_HEIGHT;
  /* stride = pixels * bits per pixel + ctrl bytes */
  display.geometry.stride       =
    display.geometry.width * LS013B7DH06_BITS_PER_PIXEL
    + LS013B7DH06_CONTROL_BYTES * 8;
...
/**************************************************************************//**
 * @brief  Refresh the LS013B7DH06 display device.
 *
                              ...
                              bool              enable)
{
  ...
  if (enable) {
    ...
    PAL_TimerMicroSecondsDelay(LS013B7DH06_ENABLE_WAIT_US);
  } else {
#if defined(LCD_PORT_DISP_PWR)
...
static EMSTATUS DisplayClear(void)
{
  ...
  /* SCS setup time */
  PAL_TimerMicroSecondsDelay(LS013B7DH06_SCS_SETUP_US);

  /* Send command */
  cmd = LS013B7DH06_CMD_ALL_CLEAR;
  PAL_SpiTransmit((uint8_t*) &cmd, 2);

  /* SCS hold time */
  PAL_TimerMicroSecondsDelay(LS013B7DH06_SCS_HOLD_US);
...
static EMSTATUS PixelMatrixAllocate(DISPLAY_Device_t*      device,
                                    ...
                                    DISPLAY_PixelMatrix_t *pixelMatrix)
{
  unsigned int allocSize = (width * LS013B7DH06_BITS_PER_PIXEL / 8
                            + LS013B7DH06_CONTROL_BYTES)
                           * height;

  (void) device; /* Suppress compiler warning: unused parameter. */

  if (width != LS013B7DH06_WIDTH) {
                                 ...
                                 unsigned int           height)
{
  ...
  for (i = 0; i < height; i++) {
    /* Clear line */
    memset(pByte, background, LS013B7DH06_WIDTH * LS013B7DH06_BITS_PER_PIXEL / 8);
    pByte += LS013B7DH06_WIDTH * LS013B7DH06_BITS_PER_PIXEL / 8;
                                ...
                                unsigned int           height)
{
  ...
  /* Need to adjust start row by one because LS013B7DH06 starts counting lines
  ...
  cmd = LS013B7DH06_CMD_UPDATE | (startRow << 8);
  PAL_SpiTransmit((uint8_t*) &cmd, 2);
  ...
  for ( i = 0; i < height; i++ ) {
    ...
    PAL_SpiTransmit((uint8_t*) p,
                    (LS013B7DH06_WIDTH * LS013B7DH06_BITS_PER_PIXEL) / 8
                    + LS013B7DH06_CONTROL_BYTES);
    p += ((LS013B7DH06_WIDTH * LS013B7DH06_BITS_PER_PIXEL) / 8
          + LS013B7DH06_CONTROL_BYTES) / sizeof(uint16_t);
